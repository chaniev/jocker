...
**Предусловия:** завершены этапы 03 и 05
...
### 4. Добавить controlled restart from elite archive
...
### 5. Обновить selection и progress reporting
...
### 6. Провести controlled validation

1. Прогнать canonical profile без новых механизмов.
2. Прогнать тот же профиль с adaptive mutation.
3. Прогнать тот же профиль с immigrants.
...
2. Mutation rate не выходит за bounds.
3. Immigrants создаются в заданной доле.
4. Restart срабатывает только по конфигурируемому триггеру.
5. Holdout quality не деградирует после включения stability-механизмов.

## Критерии завершения

1. Population diversity измеряется и сохраняется в артефактах.
2. Adaptive mutation, immigrants и restart реализованы как конфигурируемые механизмы.
3. Алгоритм не использует speciation на этой стадии.
4. Full stability pack проходит holdout validation.
5. По итогам этапа видна реальная динамика diversity и стагнации.
