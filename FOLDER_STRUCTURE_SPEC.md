...
│   ├── run_stage3_runtime_scope_validation.sh
│   ├── run_stage4_phase_guardrails.sh
│   ├── run_stage5_parallel_benchmark.sh
...
- `scripts/run_stage3_runtime_scope_validation.sh`: Stage-03 step-9 validation harness for runtime-policy evolution scope; reuses the Stage-0 compare harness to run `old-scope`, `critical-runtime`, and gated `full-scope` profiles, then writes a consolidated `summary.txt` with training/holdout `finalFitness` effect sizes and gate decisions under `.derivedData/stage3-runtime-scope-runs/<timestamp>/`.
