...
| 5 (Joker logic) | MVP+ strict-first pack (probe debt closed, pack red) | починить текущие strict-regressions в `joker-pack`, затем держать pack зелёным и расширять только при новых сценарных дырах |
...
| 7 (Self-play retuning) | protocol формализован, запуск не начат | выполнить latency baseline и запускать итерации по acceptance/stop/rollback gate |
...
flowchart TD
    ...
    subgraph DEBT["Macro-долги до системного Stage 7"]
      ...
      D5["Stage 5<br/>Joker logic: вернуть green `joker-pack`<br/>и стабилизировать acceptance после probe-debt closure"]
    ...
    class F0,F1,F3,F4a,F45probe done;
    class F6g,F6freeze,F7proto gate;
    class D4b,D4c,D5,Lat debt;
    class Run7 target;
```
...
- `scripts/run_joker_regression_pack.sh --list` подтверждает обновленный состав pack-а: `21` strict-тест, `probe=0` (`strict-first` pack).
- Реальный `joker-pack` strict-only прогон после cleanup (`.derivedData/joker-regression-runs/20260226-193536`) завершается с `exit 65`: harness работает, но найдено `6` strict-regressions (`JOKER-011/013/015`-связанные preferred-suit/control-reserve/anti-premium runtime+evaluator кейсы).
