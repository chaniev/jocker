...
|------|--------|--------------------------|
| 0 (Baseline) | почти завершен (baseline/compare + operational metrics, кроме 1 метрики) | определить и собрать `leftNeighborPremiumAssistRate` (event semantics + instrumentation) либо зафиксировать как Stage-0b/out-of-scope |
...
- Артефакты: `.derivedData/bot-baseline-runs/20260226-163420` (актуальный прогон с extended metrics; ранний snapshot `.derivedData/bot-baseline-runs/20260226-153653` сохранен как pre-instrumentation reference)
...
- Расширенные Stage-0 операционные метрики теперь собираются текущим harness (кроме `leftNeighborPremiumAssistRate`, см. pending ниже).
- Для loss-метрик (`*Loss`) отрицательная `Delta` означает улучшение.

### Расширенные операционные метрики baseline (`baseline-v1`, 20260226-163420)
...
- `pending`: `leftNeighborPremiumAssistRate` (нужна более точная event-level семантика; текущий `premiumAssistLoss` не дает надежный бинарный assist-event счетчик)
...
- [x] Собраны расширенные Stage-0 операционные метрики (кроме `leftNeighborPremiumAssistRate`, см. отдельный пункт ниже).
- [x] Добавлен краткий baseline-report с выводами по слабым местам (по данным `compare-v1` primary/holdout и сценариев).
- [ ] Определена event-level семантика и собран `leftNeighborPremiumAssistRate` (или явно вынесен из Stage 0 как follow-up).
