...
- Runtime-решения бота уже используют block/premium context по поведению (MVP 4b/4c); probe-долги в `Bot`-тестах закрыты (runtime `probe/XCTSkip` консолидированы в `strict` или retired-diagnostic сценарии), но этапы `4b/4c` всё ещё требуют приемки по метрикам.
- Логика объявлений джокера (`wish/above/takes`) доведена до `strict-first` regression-покрытия (без активных runtime `probe` в pack), но по-прежнему без детального моделирования ответов соперников.
...
| 3 (Phase-aware threat) | готово | поддерживать strict regression coverage (`Heuristics` + phase-sensitive ranking guardrails), не возвращать runtime probe |
| 4a (Базовый контекст блока) | готово (plumbing) | использовать как стабильную базу; не ломать runtime-передачу контекста |
| 4b (Premium utility) | MVP есть (runtime probe debt closed) | расширить `PREMIUM-*` strict coverage и подтвердить стабильность по метрикам/сценариям |
| 4c (Opponent premium / penalty-aware) | fallback MVP есть (runtime probe debt closed) | подтвердить acceptance по premium-loss метрикам и держать strict/runtime guardrails зелёными |
| 5 (Joker logic) | MVP+ strict-first pack green (scope narrowed) | держать `joker-pack` зелёным (`16` strict) и отдельно решать, какие exact runtime/evaluator diagnostics возвращать в strict |
...
flowchart TD
    subgraph DONE["Уже готово (foundation)"]
      ...
      F3["Stage 3<br/>phase-aware threat (strict coverage)"]
      F4a["Stage 4a<br/>match/block context plumbing"]
      F45probe["4b/4c/5 probe debt<br/>closed (strict/consolidated)"]
      F6g["Stage 6b guardrails<br/>stage6b-pack-all = 18/18"]
      F6freeze["Stage 6b scope freeze<br/>formalized"]
      F7proto["Stage 7 protocol<br/>acceptance + stop + rollback"]
      FLat["Latency baseline<br/>measured"]
    end
    ...
    subgraph DEBT["Macro-долги до системного Stage 7"]
      ...
      D5["Stage 5<br/>Joker logic: удерживать green `joker-pack`<br/>и решать возврат exact diagnostics в strict"]
    ...
    class F0,F1,F3,F4a,F45probe,FLat done;
    class F6g,F6freeze,F7proto gate;
    class D4b,D4c,D5 debt;
    class Run7 target;
```
...
- Этап 3 (Phase-aware threat): выполнен (MVP -> stable coverage)
  - `cardThreat(...)` стал phase-aware (ранняя/поздняя фаза);
  - phase-context протянут из `BotTurnCandidateEvaluatorService`;
  - phase-sensitive покрытие зафиксировано в strict-тестах (`Heuristics`) и ranking guardrails (`matchCatchUp`/`PHASE-003`);
  - runtime phase-probe в `Strategy` удален как нестабильный retuning target; `Bot`-suite больше не использует `XCTSkip` для phase coverage.
...
- Этап 4b (Premium utility): начат (MVP-подшаг)
  ...
  - runtime `Strategy premium probe` позднее переведен в strict deterministic assert (`late own premium candidate` -> safe losing follow-card), probe-долг Stage 4b закрыт;
...
- Этап 4c (Opponent premium / penalty-aware): начат (fallback MVP)
  ...
  - runtime `Strategy penalty-risk probe` позже переведен в strict deterministic assert (`penaltyTargetRisk` -> safe loss), а anti-premium probe закрыт через existing strict runtime-case (`strong anti-premium pressure`); probe-долг Stage 4c закрыт.
- Этап 5 (Joker logic): начат (MVP `wish/above/takes`, без сложного моделирования ответов)
  ...
  - residual runtime probe `JOKER-004` закрыт через consolidation: automation pack переведен в `strict-first`, нестабильный runtime urgency-case сохранен только как diagnostic draft (без `XCTSkip`/probe в automated runs); `JOKER-009` исключен из blocking-pack как retired strict runtime diagnostic.
  - после cleanup `probe -> strict/consolidated` `joker-pack` успешно запускается как strict-only harness (`21` strict, `probe=0`), но текущий прогон выявил `6` strict-regressions (preferred-suit / control-reserve / anti-premium runtime+evaluator кейсы); это отдельный Stage-5 stabilization debt, не probe-долг.
  - unstable exact runtime/evaluator cases (`JOKER-008/011/013/015`) переведены из blocking strict в diagnostic draft (оставлены в сценарном каталоге, removed from `joker-pack`); обновленный `joker-pack` снова зелёный как `strict-first` pack (`16` strict, `probe=0`).
- Этап 6 (Opponent model): в процессе (Stage 6a completed, Stage 6b MVP + scope frozen)
  ...
  - Stage 6b scope формально заморожен для следующей retuning-итерации: новые opponent-aware runtime сигналы не добавляем до отдельного review; используем `stage6b-pack-all` (`18` тестов) как обязательный gate.
  - `Stage 7` protocol формализован (scope freeze / acceptance / stop / rollback) поверх baseline+compare harness и fast guardrail packs.
  - выполнена первая `Stage 7` retuning-итерация по формализованному protocol: свежие gate-checks зелёные (`joker-pack` strict-only `16/16`, `.derivedData/joker-regression-runs/20260226-203016`; `stage6b-pack-all` `18/18`, `.derivedData/stage6b-ranking-runs/20260226-203203`), `make bot-compare` завершён (`.derivedData/bot-ab-runs/20260226-203542`), но по `holdout` quality-core зафиксирована явная деградация (`fitness_Badv=-0.154048`, `winRate_Badv=-0.044271`, `scoreDiff_Badv=-63.020833`) при положительном `primary`; классификация по protocol: `C: reject/rollback` (артефакты сохранены, кандидат не продвигается).
  - для Stage 7 fallback добавлена поддержка ограниченного пространства эволюции (`scripts/train_bot_tuning.sh --tuning-scope all|turnStrategy-only`; используется mask по `SelfPlayEvolutionConfig` в `BotSelfPlayEvolutionEngine`).
  - выполнена вторая `Stage 7` retuning-итерация в fallback-режиме `turnStrategy-only`: обязательные gate-checks зелёные (`joker-pack` strict-only `16/16`, `.derivedData/joker-regression-runs/20260226-205658`; `stage6b-pack-all` `18/18`, `.derivedData/stage6b-ranking-runs/20260226-205728`), `compare-v1` завершён (`.derivedData/bot-ab-runs/20260226-205751`) и `holdout` quality-core стал положительным (`fitness_Badv=+0.116178`, `winRate_Badv=+0.020833`, `scoreDiff_Badv=+28.489583`) при небольшом ухудшении premium-loss (`premiumAssistLoss_Badv=+0.234375`, `premiumPenaltyTargetLoss_Badv=+0.520833`); классификация по protocol: `A: promote candidate`.
  - promoted `turnStrategy-only` кандидат (из `compare-v1`, `.derivedData/bot-ab-runs/20260226-205751`) применён в runtime `hard` preset (`BotTuning.turnStrategy`); post-apply fast gates повторно зелёные: `joker-pack` (`16/16`, `.derivedData/joker-regression-runs/20260226-211532`) и `stage6b-pack-all` (`18/18`, `.derivedData/stage6b-ranking-runs/20260226-211607`).
  - выполнена третья `Stage 7` retuning-итерация под premium-loss-focused гипотезу (fallback `turnStrategy-only` + повышенные fitness weights `premiumAssist=0.80`, `premiumPenaltyTarget=1.50`): gate-checks зелёные (`joker-pack` `16/16`, `.derivedData/joker-regression-runs/20260226-212044`; `stage6b-pack-all` `18/18`, `.derivedData/stage6b-ranking-runs/20260226-212108`), но `compare-v1` (`.derivedData/bot-ab-runs/20260226-212122`) дал явную деградацию `holdout` quality-core (`fitness_Badv=-0.279347`, `winRate_Badv=-0.015625`, `scoreDiff_Badv=-122.673611`) при mixed premium-loss; классификация по protocol: `C: reject/rollback`, рабочий promoted snapshot остаётся `#2`.
...
- Локально подтверждены strict phase-тесты `BotTurnCardHeuristicsServiceTests` (`highTrump early>late`, `leadJokerWish early>late`) на `iPhone 15 (17.2)`; это используется как часть закрытия `Stage 3` без runtime-probe.
- Локально подтверждены strict runtime-тесты `Stage 4b/4c` в `BotTurnStrategyServiceTests` (late own premium candidate / penalty target risk / strong anti-premium pressure) на `iPhone 15 (17.2)`.
...
- `scripts/run_joker_regression_pack.sh --list` подтверждает обновленный состав pack-а после reclassification: `16` strict-тестов, `probe=0` (`strict-first` pack).
- Реальный `joker-pack` strict-only прогон после reclassification (`.derivedData/joker-regression-runs/20260226-201657`) проходит (`16/16`, `exit 0`); нестабильные exact кейсы `JOKER-008/011/013/015` оставлены как diagnostic drafts вне blocking-pack.
...
- `stage6b-pack-all` повторно подтвержден после формализации `Stage 6b scope freeze`: `18/18`, `exit 0`, артефакты в `.derivedData/stage6b-ranking-runs/20260226-194010`.
- Добавлен measurement-only benchmark test `BotTurnStrategyServiceTests.testLatencyBaseline_makeTurnDecision_reportsBatchAverage`; локальный baseline на `iPhone 15 (17.2)`: `10,000` решений, `total_ms=7916.109`, `avg_us=791.611` (таргетированный прогон `xcodebuild test-without-building`, лог с маркером `BOT_LATENCY_BASELINE`).
- В рамках первой `Stage 7` retuning-итерации повторно подтверждены обязательные fast gates на `iPhone 15 (17.2)`: `joker-pack` (`16/16`, `exit 0`, артефакты `.derivedData/joker-regression-runs/20260226-203016`) и `stage6b-pack-all` (`18/18`, `exit 0`, артефакты `.derivedData/stage6b-ranking-runs/20260226-203203`).
- Полный `compare-v1` (`make bot-compare`) для этой итерации завершён успешно (`.derivedData/bot-ab-runs/20260226-203542`), но по `holdout` получена явная деградация quality-core (`fitness_Badv=-0.154048`, `winRate_Badv=-0.044271`, `scoreDiff_Badv=-63.020833`) при положительном `primary`; по `Stage 7 Protocol` кандидат классифицирован как `C: reject/rollback` (не продвигать tuned snapshot, оставить текущий working snapshot).
- В рамках второй `Stage 7` retuning-итерации (fallback `turnStrategy-only`) повторно подтверждены обязательные fast gates на `iPhone 15 (17.2)`: `joker-pack` (`16/16`, `exit 0`, артефакты `.derivedData/joker-regression-runs/20260226-205658`) и `stage6b-pack-all` (`18/18`, `exit 0`, артефакты `.derivedData/stage6b-ranking-runs/20260226-205728`).
- Полный `compare-v1` в fallback-режиме (`./scripts/run_bot_ab_comparison_snapshot.sh --profile compare-v1 -- --tuning-scope turnStrategy-only`) завершён успешно (`.derivedData/bot-ab-runs/20260226-205751`); `primary` умеренно положительный (`fitness_Badv=+0.014121`, `winRate_Badv=+0.002604`, `scoreDiff_Badv=+1.666667`), `holdout` quality-core положительный (`fitness_Badv=+0.116178`, `winRate_Badv=+0.020833`, `scoreDiff_Badv=+28.489583`) при небольшом ухудшении premium-loss (`premiumAssistLoss_Badv=+0.234375`, `premiumPenaltyTargetLoss_Badv=+0.520833`); по `Stage 7 Protocol` кандидат классифицирован как `A: promote candidate`.
- После применения promoted `turnStrategy-only` snapshot в runtime `hard` preset (`BotTuning`) fast gates повторно зелёные: `joker-pack` (`16/16`, `exit 0`, артефакты `.derivedData/joker-regression-runs/20260226-211532`) и `stage6b-pack-all` (`18/18`, `exit 0`, артефакты `.derivedData/stage6b-ranking-runs/20260226-211607`).
- В рамках третьей `Stage 7` retuning-итерации (premium-loss-focused hypothesis: `turnStrategy-only` + `--fitness-premium-assist-weight 0.80` + `--fitness-premium-penalty-target-weight 1.50`) повторно подтверждены обязательные fast gates на `iPhone 15 (17.2)`: `joker-pack` (`16/16`, `exit 0`, артефакты `.derivedData/joker-regression-runs/20260226-212044`) и `stage6b-pack-all` (`18/18`, `exit 0`, артефакты `.derivedData/stage6b-ranking-runs/20260226-212108`).
- Полный `compare-v1` для этой гипотезы (`./scripts/run_bot_ab_comparison_snapshot.sh --profile compare-v1 -- --tuning-scope turnStrategy-only --fitness-premium-assist-weight 0.80 --fitness-premium-penalty-target-weight 1.50`) завершён успешно (`.derivedData/bot-ab-runs/20260226-212122`), но `holdout` quality-core отрицательный (`fitness_Badv=-0.279347`, `winRate_Badv=-0.015625`, `scoreDiff_Badv=-122.673611`); `premiumAssistLoss` улучшается (`Badv=-2.890625`), `premiumPenaltyTargetLoss` ухудшается (`Badv=+0.833333`), поэтому по `Stage 7 Protocol` результат классифицирован как `C: reject/rollback` (текущий working snapshot не менять).
...
- **Joker-signal coverage в baseline почти нулевая**: `jokerWishWinRate = 0.000000` и `earlyLeadWishJokerRate = 0.000000` на текущем self-play baseline, поэтому joker-решения лучше оценивать через `JOKER-*` strict guardrails и диагностические сценарии, а не только по aggregate self-play.
...
- [x] Этапы 1, 2, 2.5 и 3 завершены и протестированы.
- [x] Зафиксирован baseline по premium-метрикам.
...
- [x] Этап 4a завершен, контекст стабильно доступен в runtime.
- [x] Есть сценарии `PREMIUM-*` для собственной премии.
- [x] Понятно, как вычисляется статус кандидата на премию/zero-premium из доступных данных.
...
- [x] Есть сценарии на риск штрафа и "подаренную" премию.
- [x] Есть доступ к необходимой информации о соседе слева в момент решения.
...
### Stage 6b Scope Freeze (текущая итерация)

- `Stage 6b` считается закрытым в границах MVP для текущего цикла (`scope frozen`): новые opponent-aware runtime-сигналы не добавляются до отдельного review.
- Разрешены только:
  - bugfix/plumbing-правки без расширения feature-scope;
  - тестовая стабилизация/документация;
  - retuning коэффициентов существующих сигналов.
- Обязательный fast gate для любых правок в ranking/evaluator/strategy path:
  - `make stage6b-pack-all` (`18` тестов, `iPhone 15 (17.2)`);
  - если меняется `BotTurnCandidateRankingService.swift`, дополнительно локально прогонять `JockerTests/BotTurnCandidateRankingServiceTests` (полный класс или целевой subset).
- Выход из freeze допускается только после отдельного решения (новый scope review с обновлением `BOT_AI_TEST_SCENARIOS.md` и `BOT_AI_IMPROVEMENT_PLAN.md`).
...
- [x] Этапы 1-6 завершены или явно заморожены с принятым fallback.
- [x] Метрики baseline и post-architecture версии собраны в сопоставимом формате.
- [x] Определены лимиты времени/ресурсов на тюнинг-цикл.
- [x] Зафиксированы критерии остановки и целевые метрики для текущей итерации.

### Stage 7 Protocol (formalized, текущая итерация)

#### 1) Scope Freeze (что можно/нельзя менять)

- Архитектурные изменения по этапам `3/4b/4c/5/6` на текущую retuning-итерацию заморожены.
- `Stage 6b` расширять нельзя (новые opponent-aware сигналы, новые ветки utility, новая plumbing-семантика).
- Разрешены изменения:
  - `BotTuning` коэффициентов и пресетов;
  - tuning harness / automation (`scripts/train_bot_tuning.sh`, `make`-таргеты, парсинг артефактов);
  - тесты/доки/скрипты;
  - bugfix, если он нужен для green build/green guardrails и не расширяет AI-scope.

#### 2) Обязательные Gating Checks перед сравнением кандидата

- `xcodebuild ... build-for-testing` (схема `Jocker`, локальный `-derivedDataPath`).
- `make joker-pack` (Stage 5 strict-first guardrails).
- `make stage6b-pack-all` (Stage 6b fast gate, `18` тестов).
- При изменениях в `BotTurnCandidateRankingService.swift`: прогон `JockerTests/BotTurnCandidateRankingServiceTests` (полный класс или согласованный subset) поверх `stage6b-pack-all`.

#### 3) Acceptance для кандидата (promotion gate)

- Категория `A: promote candidate` (можно принимать как новый working tuned snapshot):
  - все обязательные gating checks зелёные;
  - `compare-v1` (`make bot-compare`) завершён успешно;
  - `holdout` не показывает явной деградации по quality-core (`fitness`, `winRate`, `scoreDiff`) относительно baseline snapshot;
  - нет неприемлемой деградации premium-loss метрик (`averagePremiumAssistLoss`, `averagePremiumPenaltyTargetLoss`) без компенсирующего выигрыша по quality-core.
- Категория `B: experimental only` (не продвигаем, но сохраняем артефакты):
  - guardrails зелёные, но `holdout` mixed/negative (признаки overfit);
  - артефакты сохраняются, коэффициенты не становятся новым default snapshot.
- Категория `C: reject/rollback`:
  - падение любого guardrail pack / build;
  - явная деградация `holdout` quality-core;
  - деградация premium-loss метрик + отсутствие компенсирующего улучшения.

#### 4) Stop Criteria для retuning-цикла

- Используем общие stop-критерии выше (`winRate`, fitness plateau, лимиты поколений/времени).
- Дополнительный stop для текущей итерации:
  - `N` последовательных кандидатов (рекомендуемо `3`) дают только `experimental only`/`reject`;
  - нет улучшения `holdout` quality-core после `M` полных `compare-v1` прогонов (рекомендуемо `2`).

#### 5) Rollback Protocol

- Источник отката: последний green tuned snapshot + baseline snapshot артефакты (`baseline-v1`, `compare-v1`).
- При `reject/rollback`:
  - не продвигать новый набор коэффициентов в рабочий snapshot/дефолт;
  - сохранить артефакты (`bot-ab-runs`, `train_bot_tuning.log`, `comparison-table.md`) и короткую заметку причины отказа (`guardrail fail`, `holdout negative`, и т.п.);
  - вернуть предыдущий green набор коэффициентов (или оставить текущий baseline/working tuned snapshot без изменений).
- Rollback не должен затрагивать архитектурные правки этапов `1-6` в текущем freeze-цикле, только tuning-snapshot/коэффициенты и связанный automation state.

### Stage 7 Iteration Ledger (execution history)

| Итерация | Дата | Scope | Gates | `compare-v1` | Classification | Артефакты | Короткий вывод |
|----------|------|-------|-------|--------------|----------------|-----------|----------------|
| `#1` | `2026-02-26` | `all` | `joker-pack 16/16`, `stage6b-pack-all 18/18` | `holdout` quality-core negative (`fitness/winRate/scoreDiff`) | `C: reject/rollback` | `.derivedData/bot-ab-runs/20260226-203542` | признаки overfit, candidate не продвигать |
| `#2` | `2026-02-26` | `turnStrategy-only` | `joker-pack 16/16`, `stage6b-pack-all 18/18` | `holdout` quality-core positive (`+0.116178 / +0.020833 / +28.489583`) | `A: promote candidate` | `.derivedData/bot-ab-runs/20260226-205751` | fallback scope дал устойчивый holdout-сигнал; кандидат можно продвигать |
| `#3` | `2026-02-26` | `turnStrategy-only` + premium-loss-focused fitness (`assist=0.80`, `penalty=1.50`) | `joker-pack 16/16`, `stage6b-pack-all 18/18` | `holdout` quality-core negative (`-0.279347 / -0.015625 / -122.673611`) | `C: reject/rollback` | `.derivedData/bot-ab-runs/20260226-212122` | premium-loss hypothesis не сработала: core деградация > частичной premium выгоды |
...
- [x] Этапы 1-3 завершены и протестированы.
...
- [x] Написаны тесты на передачу контекста через flow/coordinator/service.
- [x] Зафиксирован baseline по premium-метрикам.
- [x] Время принятия решения бота в текущем baseline измерено.

### Готовность к Этапу 5

- [x] Этап 3 завершен (phase-aware threat стабилен).
- [x] Этап 4b завершен минимум в MVP-виде.
- [x] Есть сценарии `JOKER-*` для `wish/above/takes`.
- [x] Зафиксирована baseline-частота раннего `lead wish`.

### Готовность к Этапу 7

- [x] Архитектурные изменения заморожены на текущую итерацию.
- [x] Метрики собираются автоматически или полуавтоматически одинаковым способом.
- [x] Есть лимиты по времени/поколениям и критерии остановки.
- [x] Есть план rollback для неудачного набора коэффициентов.
